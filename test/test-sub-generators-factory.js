...
describe('FactorySubGenerator', () => {
  ...
  describe('writing', () => {
    it('should have the initializing called with the right stuff', () => {
      ...
      let _firstCall = ['factory.js', knownPaths.PATH_CLIENT_FEATURES + _gen.options.feature + '//factory/' + _gen.name + '.factory.js', {name: utils.capitalizeFirst(_gen.name)}];
      let _secondCall = ['factory_test.js', knownPaths.PATH_CLIENT_FEATURES_TEST + _gen.options.feature + '//factory/' + _gen.name + '.factory_test.js', {name: utils.capitalizeFirst(_gen.name)}];
