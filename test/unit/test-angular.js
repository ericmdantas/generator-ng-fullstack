...
describe('ng2', () => {
  ...
  describe('copyDirective', () => {
    it('should call the methods with the right params', () => {
      ...
      let _firstCall = ['ng2/directive.ts', knownPaths.PATH_CLIENT_FEATURES + _gen.options.feature + '/' + _gen.name + '.ts', {name: _gen.name}];
      let _secondCall = ['ng2/directive_test.ts', knownPaths.PATH_CLIENT_FEATURES_TEST + _gen.options.feature + '/' + _gen.name + '_test.ts', {name: _gen.name}];
  ...
  describe('copyFactory', () => {
    it('should have the initializing called with the right stuff', () => {
      ...
      let _firstCall = ['ng2/factory.ts', knownPaths.PATH_CLIENT_FEATURES + _gen.options.feature + '/' + _gen.name + '.ts', {name: utils.capitalizeFirst(_gen.name)}];
      let _secondCall = ['ng2/factory_test.ts', knownPaths.PATH_CLIENT_FEATURES_TEST + _gen.options.feature + '/' + _gen.name + '_test.ts', {name: utils.capitalizeFirst(_gen.name)}];
